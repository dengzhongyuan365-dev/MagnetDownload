# 磁力链接下载完整流程

---

## 🔄 完整的下载流程

### 流程图（Mermaid）

```mermaid
flowchart TD
    Start([用户输入磁力链接]) --> Parse[步骤1: 解析磁力链接]
    Parse --> |InfoHash| DHT[步骤2: DHT查找Peer 需要Bencode]
    DHT --> |Peer列表| Connect[步骤3: 连接Peer]
    Connect --> Metadata[步骤4: 获取元数据 需要Bencode]
    Metadata --> Download[步骤5: 下载文件数据]
    Download --> End([下载完成])
    
    style Parse fill:#e1f5ff
    style DHT fill:#ffe1e1
    style Connect fill:#e1f5ff
    style Metadata fill:#ffe1e1
    style Download fill:#e1f5ff
    style Start fill:#d4edda
    style End fill:#d4edda
```

### 时序图（Mermaid）

```mermaid
sequenceDiagram
    participant User as 用户
    participant Parser as MagnetUriParser
    participant DHT as DHTClient
    participant Bencode as Bencode
    participant UDP as UdpClient
    participant DHTNode as DHT节点
    participant Peer as Peer节点
    participant PeerMgr as PeerManager
    
    User->>Parser: 磁力链接
    Parser->>Parser: 解析 URI
    Parser-->>User: InfoHash
    
    User->>DHT: find_peers(InfoHash)
    DHT->>Bencode: encode(查询消息)
    Bencode-->>DHT: Bencode 数据
    DHT->>UDP: send(查询)
    UDP->>DHTNode: UDP 数据包
    
    DHTNode->>UDP: 响应数据包
    UDP->>DHT: 接收响应
    DHT->>Bencode: decode(响应)
    Bencode-->>DHT: Peer 列表
    DHT-->>User: Peer 列表
    
    User->>PeerMgr: connect(Peer)
    PeerMgr->>Peer: TCP 连接
    PeerMgr->>Peer: BitTorrent 握手
    PeerMgr->>Peer: 请求元数据
    
    Peer->>PeerMgr: 元数据(Bencode)
    PeerMgr->>Bencode: decode(元数据)
    Bencode-->>PeerMgr: 文件信息
    PeerMgr-->>User: 文件信息
    
    User->>PeerMgr: 开始下载
    loop 下载每个 Piece
        PeerMgr->>Peer: 请求 Piece
        Peer->>PeerMgr: Piece 数据
        PeerMgr->>PeerMgr: 验证哈希
        PeerMgr->>PeerMgr: 写入文件
    end
    
    PeerMgr-->>User: 下载完成
```

---

## 📊 模块依赖关系

### 架构图（Mermaid）

```mermaid
graph TB
    subgraph APP[应用层]
        DM[DownloadManager]
    end
    
    subgraph BIZ[业务层]
        PM[PeerManager]
        DHT[DHTClient]
        Parser[MagnetUriParser]
    end
    
    subgraph PROTO[协议层]
        Bencode[Bencode 必需]
        BT[BitTorrent]
    end
    
    subgraph NET[网络层]
        UDP[UdpClient]
        TCP[TcpConnection]
    end
    
    subgraph BASE[基础层]
        ELM[EventLoopManager]
        Logger[Logger]
    end
    
    DM --> PM
    DM --> DHT
    DM --> Parser
    
    PM --> BT
    PM --> TCP
    PM --> Bencode
    
    DHT --> Bencode
    DHT --> UDP
    
    BT --> Bencode
    
    UDP --> ELM
    TCP --> ELM
    
    PM --> Logger
    DHT --> Logger
    
    style Bencode fill:#ffe1e1,stroke:#ff0000,stroke-width:3px
    style DM fill:#e1f5ff
    style PM fill:#e1f5ff
    style DHT fill:#e1f5ff
    style Parser fill:#e1f5ff
```

### 依赖关系详解

```mermaid
graph LR
    A[DownloadManager] -->|使用| B[PeerManager]
    A -->|使用| C[DHTClient]
    A -->|使用| D[MagnetUriParser]
    
    B -->|使用| E[Bencode]
    B -->|使用| F[TcpConnection]
    
    C -->|使用| E
    C -->|使用| G[UdpClient]
    
    E -.->|核心依赖| H[DHT协议]
    E -.->|核心依赖| I[元数据解析]
    
    G -->|使用| J[EventLoopManager]
    F -->|使用| J
    
    style E fill:#ffe1e1,stroke:#ff0000,stroke-width:3px
    style H fill:#fff3cd
    style I fill:#fff3cd
```

### 最小可工作系统

要实现磁力链接下载，**最少需要**：

```
✅ MagnetUriParser  - 解析磁力链接
✅ Bencode          - 编解码 DHT 消息和元数据
✅ UdpClient        - UDP 通信
✅ DHTClient        - 查找 Peer
✅ TcpConnection    - 连接 Peer
✅ PeerManager      - 管理 Peer 和下载数据
```

